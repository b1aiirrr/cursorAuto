...
class RewardsEngine:
    ...
    async def sweep_idle_to_earn(self, min_balance_usdt: float = 10.0, target_apr: float = 15.0) -> dict | None:
        try:
            ...
            if usdt_balance < min_balance_usdt:
                return None
            subscribe_amount = usdt_balance * 0.95
            ...
            if result.get("success"):
                await self.state.add_log("info", f"Yield Engine: Successfully swept ${subscribe_amount:.2f} USDT into Simple Earn (APR: {target_apr}%)")
            else:
                await self.state.add_log("warning", f"Yield Engine: Simple Earn subscription failed: {result.get('message')}")
            return {"action": "sweep_earn", "amount_usdt": subscribe_amount, "apr": target_apr, "timestamp": datetime.utcnow().isoformat()}
    ...
    async def check_and_subscribe_dual_investment(self) -> dict | None:
        try:
            async with httpx.AsyncClient(timeout=30.0) as http_client:
                ...
                response = await http_client.get("https://www.binance.com/sapi/v1/dci/product/list", headers=headers)
                if response.status_code == 200:
                    products = response.json().get("data", [])
                    if products:
                        product = products[0]
                        await self.state.add_log("info", f" Dual Investment available: {product.get('asset', 'BTC')}")
                        return {"action": "dual_investment_check", "product": product.get("asset"), "timestamp": datetime.utcnow().isoformat()}
        except Exception as e:
            ...
            if response and response.get("code") == "000000":
                ...
                for pool in pools:
                    if pool.get("status") == "ACTIVE":
                        ...
                        available_pools.append({"pool_name": pool_name, "token": token_name, "apr": round(apr, 2), "status": "ACTIVE"})
                        await self.state.add_log("info", f" Launchpool detected: {token_name} | APR: {apr:.1f}%")
        except Exception as e:
            logger.error(f"Launchpool Check Error: {e}")
        return available_pools

    async def stake_for_launchpool(self, pool_token: str, amount_bnb: float) -> dict | None:
        if amount_bnb < 0.01: return None
        try:
            params = {"poolToken": pool_token, "amount": str(amount_bnb)}
            ...
            if result and result.get("code") == "000000":
                await self.state.add_log("info", f" Launchpool: Successfully staked {amount_bnb} BNB for {pool_token}")
                return {"action": "launchpool_stake", "pool": pool_token, "amount_bnb": amount_bnb, "timestamp": datetime.utcnow().isoformat()}
            else:
                await self.state.add_log("warning", f" Launchpool: Staking failed: {result.get('message', 'Unknown error')}")
        except Exception as e:
    ...
    async def track_referral_performance(self, post_content: str) -> dict:
        ...
        if has_referral:
            ...
            await self.state.add_log("info", f" Referral CTA detected in post | Total CTAs shown: {self.referral_clicks}")
        return {"referral_in_post": has_referral, "total_ctas": self.referral_clicks, "conversions": self.referral_conversions, "timestamp": datetime.utcnow().isoformat()}

    async def get_best_performing_tickers(self, post_history: list) -> list[str]:
        ticker_counts = {}
        for post in post_history[-50:]:
    ...
    async def check_daily_claims(self) -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as http_client:
                headers = {"X-Square-OpenAPI-Key": self.client.API_KEY}
                earn_response = await http_client.get("https://www.binance.com/sapi/v1/simple-earn/flexible/positions", headers=headers)
                if earn_response.status_code == 200:
                    ...
                    for pos in positions:
                        ...
                        claimable.append({"type": "simple_earn", "asset": asset, "amount": amount, "apr": round(apr, 2)})
                if claimable:
                    await self.state.add_log("info", f" Daily Claims: Found {len(claimable)} active earning positions")
        except Exception as e:
            await self.state.add_log("error", f"Daily Claims Check Error: {str(e)}")
        return {"claimable_rewards": claimable, "total_positions": len(claimable), "timestamp": datetime.utcnow().isoformat()}

    async def run_reward_cycle(self, post_history: list) -> dict:
        results = {"yield_sweep": None, "dual_investment": None, "launchpools": [], "referral_stats": {}, "daily_claims": {}, "best_tickers": []}
        tasks = [self.sweep_idle_to_earn(), self.check_daily_claims(), self.check_launchpools(), self.get_best_performing_tickers(post_history)]
        yield_result, claims_result, pools_result, best_tickers = await asyncio.gather(*tasks, return_exceptions=True)
        if isinstance(yield_result, dict): results["yield_sweep"] = yield_result
        if isinstance(claims_result, dict): results["daily_claims"] = claims_result
        if isinstance(pools_result, list): results["launchpools"] = pools_result
        if isinstance(best_tickers, list): results["best_tickers"] = best_tickers
        return results
